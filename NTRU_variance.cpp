...
int N = 7, p = 3, q = 128, d = 2; // Parameters chosen by trusted party.
int attemp = 100;
bool found = false;
...
-----------------------------------------*/
    ...
    //Ring_p candidate_2(N, p, generate_coeffs);
    Ring_p candidate_1(N, q, {1, 0, 3, -3, -3, 0, 3}); // f = 1 + p*F, F \in \mathcal{T}(d,d)
    Ring_p candidate_2(N, p, {1, 0, 3, -3, -3, 0, 3}); 
    try {
        Ring_p inverse_1 = candidate_1.inverse_ppower();
...
Ring_p target_g(N, q, {0, -3, -3, 0, 3, 0, 3}); // g \in R_q
Ring_p F_q = target_f_q.inverse_ppower(); // inverse f^{-1} in R_q
//cout << "F_q(x): "; F_q.print();
Ring_p F_p(N, p, {1});
...
Ring_p mess(N, q, {1, -1, 1, 1, 0, -1, 0});
//Ring_p mess = generate_r(N, q, d);
cout << "message: "; mess.print(); cout << endl;
Ring_p random(N, q, {-1, 1, 0, 0, 0, -1, 1});
//Ring_p random = generate_r(N, q, d);

Ring_p cipher_e = (random * h) + mess;
...
cout << "Alice encrypts the message as a(x): "; a_2.center_lift().print();
...
ofstream fout("./test_data/NTRUori_vs_var_v.txt");
for (int i = 0; i < 500; ++i) {
    fout << time[i] << "\n";
}
fout.close();
